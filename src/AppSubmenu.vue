        ...
        <template v-for="(item,i) of items">
            <li v-if="visible(item) && !item.separator" :key="item.label || i" :class="[item.badgeStyle, {'layout-root-menuitem': root, 'active-menuitem': activeIndex === i && !item.disabled}]"
                ...
                <transition name="layout-submenu-container">
                    <AppSubmenu v-show="item.items && (root ? true : activeIndex === i)"  :items="visible(item) && item.items" :menuActive="menuActive" :layoutMode="layoutMode" @menuitem-click="$emit('menuitem-click', $event)"></AppSubmenu>
                </transition>
            </li>
